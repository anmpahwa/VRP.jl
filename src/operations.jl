...
function insertnode!(c::CustomerNode, nᵗ::Node, nʰ::Node, r::Route, s::Solution)
    ...
    aᵗ = s.A[(nᵗ.iⁿ, c.iⁿ)]
    aʰ = s.A[(c.iⁿ, nʰ.iⁿ)]
    nᵖ = isdelivery(c) ? s.C[c.jⁿ] : s.C[c.iⁿ] 
    nᵈ = isdelivery(c) ? s.C[c.iⁿ] : s.C[c.jⁿ]
    # update associated customer nodes
    s.πᶠ -= 0.
    s.πᵒ -= 0.
    s.πᵖ -= !isequal(nᵖ.r, nᵈ.r) * abs(c.qᶜ)
    s.πᵖ -= abs(c.qᶜ)
    c.jⁿ  = c.jⁿ
    c.iʳ  = r.iʳ
    c.iᵛ  = r.iᵛ
        ...
        for r ∈ v.R
            ...
            if isopt(r)
                ...
                while true
                    ...
                    tᵈ = c.tᵈ
                    n += 1
                    q += ispickup(c) * abs(c.qᶜ) - isdelivery(c) * abs(c.qᶜ)
                    l += s.A[(c.iⁿ,c.iʰ)].l
                    c  = s.C[c.iʰ]
                end
                r.θᵉ = r.θˢ - r.l/v.lᵛ
...
function postremove!(s::Solution)
    ...
    for c ∈ s.C
        if isdelivery(c)
            if c.jⁿ ≤ lastindex(s.D) continue end
            cᵖ = s.C[c.jⁿ]
            cᵈ = s.C[c.iⁿ]
